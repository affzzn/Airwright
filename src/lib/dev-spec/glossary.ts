...
export const GLOSSARY: GlossaryTerm[] = [
  ...
  { id: "configuration", term: "House type (configuration)", definition: "The house type's fixed build form: Detached / Semi-detached / End terrace / Mid-terrace. Each drawing IS one type (a semi is a separate drawing), so it lives on the take-off, is confirmed on the review screen, and cascades — deciding which sides are scaffolded, the corners, the apex reduction and the party wall. Plots created from the take-off inherit it; how many of a type get built is the pricing-stage QUANTITY, not a type change." },
  { id: "party-wall", term: "Party wall", definition: "A shared-wall spec item (inside apex, no rails) — its OWN unit-priced line (£165 provisional), NOT part of the apex/table-lift items. One per non-detached house (detached 0; mid-terrace still 1); removable per job." },
  ...
  { id: "adaption", term: "Adaption", definition: "On timber frame, boards temporarily pulled out and put back so trades can work. Inside-board adaption covers every adaption lift; hop-up adaption covers every adaption lift except the bottom kicker — both priced by the metre (LM). The apex is a separate UNIT cost on each. Adaption lifts = total lifts, except a 2.5-storey drops its 1 m lift (2→3, 2.5→3, 3→4).", layer: "engine" },
