...
| **Corner / return** **[reads]** | An external corner of the footprint. Scaffold must **wrap past** a corner, so Airwright adds an allowance per corner. **Count external returns only** ✅ (internal ones are not counted). Allowance quantum ⚠️ **open** — 1 m/corner is the working default, but a 5 m allowance was seen on a 4-corner rectangle; confirm with Colin. |
| **Number of lifts** **[computes]** | `ceil(height ÷ 1.5) + 1 if room-in-roof`, cross-checked against the storey template ✅. The AI supplies only the **height** and **storeys**; the engine computes the count. Which **height datum** to divide (soffit / eaves / wall plate / ridge) is ⚠️ **open**. |
| **Height to soffit** **[reads]** | The vertical height from ground to the **soffit/eaves** — the top of the wall the scaffold reaches. Read from a vertical dimension (e.g. `U/S Wallplate 4725` = 4.725 m). This is the number the lift count divides. (Exact datum ⚠️ open.) |
...
| **Cavity deduction** | The amount deducted from external dimensions to approximate the internal size when only external dims are given — **600 mm or 900 mm** ⚠️ **open** (Laura's 600 was explicitly "don't take as fact"). Affects every birdcage. Fallback when no internal dims exist is also ⚠️ open. |
