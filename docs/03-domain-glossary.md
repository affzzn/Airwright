...
| **Corner / return** **[reads]** | An external (outward) corner of the footprint. Scaffold must **wrap past** it, so Airwright adds an allowance per corner. **External corners = 4 + one per reentrant/step corner** (a plain rectangle = 4; a step or L → 5). **Count external returns only** ✅ (reentrant ones are not counted, but they signal a non-rectangle → split the birdcage). Allowance = **1 m per external corner** ✅ **CONFIRMED**. |
